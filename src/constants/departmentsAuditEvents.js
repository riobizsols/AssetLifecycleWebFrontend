export const DEPARTMENTS_APP_ID = 'DEPARTMENTS';

// The 3 events for DEPARTMENTS app are:
// - Create: When departments are created
// - Update: When departments are updated
// - Delete: When departments are deleted
...
const generateDynamicDescription = (eventName, additionalData = {}) => {
  ...
  const deptName = additionalData.deptName || additionalData.dept_name || additionalData.text;
  const orgId = additionalData.orgId || additionalData.org_id;
  const action = additionalData.action;
  const count = additionalData.count;
  const deptIds = additionalData.deptIds || additionalData.dept_ids;
  ...
  if (deptIds && Array.isArray(deptIds)) {
    description += ` - Departments: ${deptIds.join(', ')}`;
  }
