...
const bustVendorListCache = () => {
  ...
  useVendorsStore.getState().invalidateVendorsCache();
};

function readVendorAddDraft() {
  try {
    const raw = sessionStorage.getItem(VENDOR_ADD_DRAFT_KEY);
    if (!raw) return null;
...
const AddEntityForm = () => {
  ...
  // and returning never flashes / loses Vendor Details fields.
  const initialDraftRef = useRef(readVendorAddDraft());
  const initialDraft = initialDraftRef.current;
  const initialReturnTab = peekReturnTab();

  const [form, setForm] = useState(() => ({
    ...DEFAULT_VENDOR_FORM,
    ...(initialDraft?.form || {}),
    org_id: org_id || initialDraft?.form?.org_id || "",
  }));

  const [loading, setLoading] = useState(false);
  const [activeTab, setActiveTab] = useState(
    () => initialReturnTab || initialDraft?.activeTab || "Vendor Details"
  );
  ...
  const [savedTabs, setSavedTabs] = useState(
    () => new Set(Array.isArray(initialDraft?.savedTabs) ? initialDraft.savedTabs : [])
  );
  ...
  const [selectedSLAs, setSelectedSLAs] = useState(
    () => (Array.isArray(initialDraft?.selectedSLAs) ? initialDraft.selectedSLAs : [])
  );
  const [slaDropdownValue, setSlaDropdownValue] = useState('');
  ...
  const writeVendorDraft = useCallback((returnTab) => {
    const snap = draftSnapshotRef.current;
    sessionStorage.setItem(
      VENDOR_ADD_DRAFT_KEY,
      JSON.stringify({
        form: snap.form,
        selectedSLAs: snap.selectedSLAs,
        activeTab: returnTab ?? snap.activeTab,
        createdVendorId: snap.createdVendorId,
        vendorSaved: snap.vendorSaved,
        savedTabs: [...snap.savedTabs],
      })
    );
  }, []);
  ...
  const persistVendorDraft = useCallback(
    (returnTab) => {
      writeVendorDraft(returnTab);
    },
    [writeVendorDraft]
  );

  // Keep draft warm while editing so Product Details → Add Brand/Model never loses data.
  useEffect(() => {
    writeVendorDraft();
  }, [form, selectedSLAs, activeTab, createdVendorId, vendorSaved, savedTabs, writeVendorDraft]);

  // Consume return-tab flags once after navigating back from Brand/Model (or Asset Type) screens.
  useEffect(() => {
    const returnTab =
      sessionStorage.getItem("vendorProductReturnTab") ||
      sessionStorage.getItem("vendorServiceReturnTab");
    ...
    // Keep vendorAddDraft until Save All / Cancel so remounts still restore.
    // eslint-disable-next-line react-hooks/exhaustive-deps -- run once on mount
  }, []);

  // Keep org_id in sync when auth loads after first paint
  useEffect(() => {
    if (!org_id) return;
    setForm((prev) => (prev.org_id === org_id ? prev : { ...prev, org_id }));
  }, [org_id]);
