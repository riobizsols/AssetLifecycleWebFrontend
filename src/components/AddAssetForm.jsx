...
const AddAssetForm = ({ userRole }) => {
  ...
  // New unified save function that handles both asset creation and document upload
  const handleUnifiedSave = async () => {
    console.log('🚀 Starting unified save process...');
    setSubmitAttempted(true);
    ...
    try {
      let currentAssetId = form.assetId; // Initialize with existing asset ID
      
      // Step 1: Create asset if not already created
      if (!form.assetId) {
        console.log('📝 Creating asset first...');
        setIsSubmitting(true);
        ...
        const assetData = {
          ...
          serial_number: form.serialNumber,
          description: form.description,
          org_id: user?.org_id, // Add org_id from user object
          branch_id: getUserBranchId(user?.user_id), // Auto-populate user's branch
          purchase_vendor_id: form.purchaseSupply || null, // Use Purchase Vendor dropdown value
          service_vendor_id: form.serviceSupply || null, // Set from Service Vendor dropdown
          prod_serv_id: form.serviceSupply || null, // Set from Service Vendor dropdown
          maintsch_id: null, // Always set to null
          purchased_on: form.purchaseDate, // Backend expects purchased_on
          purchased_cost: form.purchaseCost, // Backend expects purchased_cost
          purchased_by: form.purchaseBy || null, // Backend expects purchased_by
          warranty_period: form.warrantyPeriod || null,
          expiry_date: form.expiryDate || null,
          current_status: form.status || 'Active', // Backend expects current_status
          parent_asset_id: form.parentAsset || null, // Parent asset for child assets
          assigned_to: form.assignedTo || null,
          properties: form.properties || {} // Include dynamic properties
        };
        ...
        // Check for success - the backend returns {message: 'Asset added successfully', asset: {...}}
        if (response.data && response.data.message && response.data.message.includes('successfully')) {
          const assetId = response.data.asset?.asset_id;
          console.log('🆔 Asset created with ID:', assetId);
          
          if (assetId) {
            currentAssetId = assetId; // Set the current asset ID
            ...
            // Try to add to print queue if it's a printer
            if (form.assetType && form.assetType.toLowerCase().includes('printer')) {
              try {
                console.log('🖨️ Adding printer to print queue...');
                const printQueueResponse = await API.post('/print-queue/add', {
                  asset_id: assetId,
                  printer_name: form.name || form.serialNumber,
                  location: form.location || 'Unknown',
                  status: 'Active'
                });
                console.log('✅ Print queue response:', printQueueResponse.data);
                toast.success('Asset created and added to print queue!');
              } catch (printError) {
                console.error('❌ Print queue error:', printError);
                // Don't fail the whole operation for print queue issues
                toast.success('Asset created successfully! (Print queue addition failed - you can add manually later)');
              }
            }
          } else {
          ...
          console.error('Asset creation failed:', response.data);
      ...
      // Step 2: Upload documents if any are selected
      console.log('📎 Document upload check:');
      console.log('📎 Attachments length:', attachments.length);
      console.log('📎 Current asset ID:', currentAssetId);
      console.log('📎 Form asset ID:', form.assetId);
      
      if (attachments.length > 0 && currentAssetId) {
        console.log('📎 Uploading documents...');
        console.log('📎 Current asset ID:', currentAssetId);
        console.log('📎 Attachments to upload:', attachments);
        setIsUploading(true);
        
        const validAttachments = attachments.filter(a => a.type && a.file);
        if (validAttachments.length === 0) {
          toast.error('Please select document types and files for all attachments');
          setIsUploading(false);
          return;
        }

        // Check for custom name requirements
        const needsCustomName = validAttachments.some(a => {
          const selectedDocType = documentTypes.find(dt => dt.id === a.type);
          return selectedDocType && (selectedDocType.text.toLowerCase().includes('other') || selectedDocType.doc_type === 'OT') && !a.docTypeName?.trim();
        });

        if (needsCustomName) {
          toast.error('Please provide custom names for "Other" document types');
          setIsUploading(false);
          return;
        }
        ...
        for (const a of validAttachments) {
          try {
            const fd = new FormData();
            fd.append('file', a.file);
            fd.append('dto_id', a.type); // Backend expects dto_id, not docTypeId
            fd.append('org_id', user?.org_id); // Add org_id which is required
            if (a.docTypeName?.trim()) {
              fd.append('doc_type_name', a.docTypeName.trim()); // Backend expects doc_type_name, not customName
            }
            
            console.log('📎 Uploading file:', a.file.name, 'with dto_id:', a.type, 'org_id:', user?.org_id);
            
            // Upload to the asset documents API
            await API.post(`/assets/${currentAssetId}/docs/upload`, fd, {
              headers: { 'Content-Type': 'multipart/form-data' }
            });
            successCount++;
      ...
      // Step 3: Navigate to assets list
      console.log('✅ Unified save completed, navigating to assets list');
      navigate('/assets');
      
    } catch (err) {
      console.error('❌ Error in unified save:', err);
      const errorMessage = err.response?.data?.error || 'Failed to save asset';
      toast.error(errorMessage);
    } finally {
      setIsSubmitting(false);
      setIsUploading(false);
  ...
  return (
    <div className="max-w-6xl mx-auto mt-8 bg-[#F5F8FA] rounded-xl shadow">
      ...
      <div className="px-8 pt-6 pb-4 border-t border-gray-200">
          ...
          >
            {isSubmitting || isUploading ? (
              <span className="flex items-center">
                ...
                {isSubmitting ? 'Saving Asset...' : 'Uploading Files...'}
              </span>
            ) : (
              t('common.save')
            )}
