      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
        <div className="flex items-center gap-4">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder={t('auditLogConfig.searchPlaceholder')}
              value={searchTerm}
          ...
          >
            <option value="">{t('auditLogConfig.allApps')}</option>
            {uniqueApps.map(app => (
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        {isLoading ? (
          <div className="flex items-center justify-center py-12">
            ...
            <span className="ml-2 text-gray-600">{t('auditLogConfig.loading')}</span>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('auditLogConfig.appAndEvent')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('auditLogConfig.status')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('auditLogConfig.reportingRequired')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('auditLogConfig.email')}</th>
                </tr>
              </thead>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-600">
                          {config.enabled ? t('auditLogConfig.enabled') : t('auditLogConfig.disabled')}
                        </span>
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-600">
                          {config.reporting_required ? t('auditLogConfig.required') : t('auditLogConfig.notRequired')}
                        </span>
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      {!isReadOnly && editingEmail === config.alc_id ? (
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            placeholder={t('auditLogConfig.enterEmailPlaceholder')}
                            autoFocus
                          />
                          <button
                            ...
                            title={t('auditLogConfig.saveEmail')}
                          >
                            {isSaving ? (
                          ...
                          <button
                            ...
                            title={t('auditLogConfig.cancelEditing')}
                          >
                            <X className="w-4 h-4" />
                          </button>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-sm text-gray-900">
                            {config.reporting_email || t('auditLogConfig.noEmailSet')}
                          </span>
                          {!isReadOnly && (
                            <button
                              ...
                              title={t('auditLogConfig.editEmail')}
                            >
        ...
        {!isLoading && filteredConfigs.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900">{t('auditLogConfig.noConfigurationsFound')}</h3>
            <p className="mt-1 text-sm text-gray-500">{t('auditLogConfig.tryAdjustingSearch')}</p>
          </div>
        )}
      </div>
    </div>
