...
const SerialNumberPrint = () => {
  ...
  // Label templates are now imported from separate file
  ...
  const fetchPrintQueue = async (status = filters.status) => {
    try {
      ...
      showBackendTextToast({ toast, tmdId: 'TMD_FAILED_TO_LOAD_PRINT_QUEUE_20C44466', fallbackText: 'Failed to load print queue', type: 'error' });
      setPrintQueue([]);
      setFilteredQueue([]);
    } finally {
      setIsLoading(false);
  ...
  const fetchPrinters = async () => {
    try {
      console.log('Fetching printers from assets by type AT017...');
      const res = await API.get('/assets/type/AT017');
      ...
      console.error('Error loading printers from assets, falling back to mock:', error);
