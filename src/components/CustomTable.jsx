...
const CustomTable = ({
  ...
  const visible = visibleColumns.filter(
    (col) => col.visible && !isIdColumnName(col.name),
  );
  ...
  const renderCellContent = (col, row) => {
    ...
    if (col.name === "expiry_status") {
      ...
      if (expiryStatus && expiryStatus.text && expiryStatus.color) {
        return (
          <span
            className={`px-2 py-1 text-xs font-medium rounded-full ${expiryStatus.color}`}
          >
            {expiryStatus.text}
          </span>
        );
  ...
  return (
    <>
      {data.map((row, rowIndex) => {
        const isRowSelected =
          selectedRows && selectedRows.includes(row[rowKey]);
        return (
          <tr
              ...
              <td key={colIndex} className="border text-xs px-4 py-2">
                {colIndex === 0 ? (
                  <div className="flex items-center gap-2">
                    ...
                    {renderCell
                      ? (renderCell(col, row) ?? renderCellContent(col, row))
