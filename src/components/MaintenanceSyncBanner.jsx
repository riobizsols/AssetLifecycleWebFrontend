...
/**
 * Sync / connectivity banner for Maintenance list & detail.
 * Reuses the shared offline connectivity store (same as inspection).
 */
export default function MaintenanceSyncBanner({ moduleLabel = 'maintenance' } = {}) {
  ...
  if (!token && offline) {
    return (
      ...
      >
        ...
        open {moduleLabel}. Offline login is not available.
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        ...
        {status === 'offline' && (
          <span className="text-xs opacity-80">
            Browse cached {moduleLabel}; saving requires an online connection
          </span>
        )}
