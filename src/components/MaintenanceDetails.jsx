...
const MaintenanceDetails = () => {
  ...
  const handleCreateJobRole = async (e) => {
    ...
    try {
      ...
      showBackendTextToast({
        toast,
        tmdId: 'TMD_FAILED_TO_ASSIGN_JOB_ROLE_0B4AAC08',
        fallbackText: error.response?.data?.error || error.response?.data?.details || 'Failed to assign job role',
        type: 'error',
      });
    }
  };
  ...
  const handleDeleteJobRole = async (id) => {
    ...
    try {
      ...
      showBackendTextToast({
        toast,
        tmdId: 'TMD_FAILED_TO_REMOVE_JOB_ROLE_62A4EC13',
        fallbackText: error.response?.data?.error || error.response?.data?.details || 'Failed to remove job role',
        type: 'error',
      });
    }
  };
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        >
                          ...
                          {workflowSteps.filter((step) => !sequences.some((seq) => seq.wf_steps_id === step.wf_steps_id)).map((step) => (
          ...
          <div>
              ...
              {selectedStepForJobRole && (
                <>
                  {/* Table Header with Actions */}
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-semibold text-gray-900 text-lg">Selected Job Roles</h3>
                    <button
                      onClick={async () => {
                        if (!newJobRole.job_role_id) {
                          showBackendTextToast({ toast, tmdId: 'TMD_PLEASE_SELECT_A_JOB_ROLE_0B641D1A', fallbackText: 'Please select a job role', type: 'error' });
                  ...
                  {/* Add Job Role Form */}
                  <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <h4 className="text-sm font-semibold text-gray-700 mb-3">Select Job Role</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">
                  ...
                  {loadingJobRoles ? (
                    ...
                    <div className="text-center py-16 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                      <p className="text-gray-500 text-lg">No job roles selected for this step</p>
                      <p className="text-sm text-gray-400 mt-2">Select job roles using the form above</p>
                    </div>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="min-w-full border border-gray-200">
