...
test.describe('RIO EAM reports', () => {
  ...
  test('loads reports, previews the register, and generates a PDF', async ({ page }) => {
    ...
    await openReport(page, '/reports/asset-report', 'Asset Register');
    ...
    await openReport(page, '/reports/asset-lifecycle-report', 'Asset Lifecycle Report');
    await expect(page.getByText('Purchase Date').first()).toBeVisible();
    await expect(page.getByText(/Preview • \d+ rows/)).toBeVisible({ timeout: 30000 });

    await openReport(page, '/reports/maintenance-history', 'Maintenance History');
    await expect(page.getByText('Work Order ID').first()).toBeVisible();
    await expect(page.getByText(/Preview • \d+ rows/)).toBeVisible({ timeout: 30000 });

    await openReport(page, '/reports/asset-valuation', 'Asset Valuation');
    ...
    await openReport(page, '/reports/breakdown-history', 'Breakdown History');
    ...
    await openReport(page, '/reports/sla-report', 'SLA Reports');
    await expect(page.getByText('SLA Description').first()).toBeVisible();
    await expect(page.getByText(/Preview • \d+ rows/)).toBeVisible({ timeout: 30000 });

    await page.goto(`${BASE}/reports/usage-based-asset`);
    ...
    await openReport(page, '/reports/asset-workflow-history', 'Asset Workflow History');
    await expect(page.getByText('Work Order ID').first()).toBeVisible();
    await expect(page.getByText(/Preview • \d+ rows/)).toBeVisible({ timeout: 30000 });

    await openReport(page, '/reports/qa-audit-report', /QA.*Audit/);
...
async function openReport(page, path, title) {
  ...
  await expect(titleLocator).toBeVisible({ timeout: 25000 });
  await expect(page.getByRole('button', { name: 'Preview' })).toBeVisible({ timeout: 15000 });
