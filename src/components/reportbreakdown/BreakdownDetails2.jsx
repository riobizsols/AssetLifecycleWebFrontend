...
const BreakdownDetails2 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const selectedReasonCode = reasonCodes.find(
                ...
                <EnhancedDropdown
                  options={[
                    ...reasonCodes.map((c) => ({
                      ...
                      label:
                        c.text || c.name || c.description || c.code || c.id,
              ...
              <div className="space-y-1">
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-lg px-4 py-3">
                  <div className="text-xs text-gray-500 mb-1">
                    {t("breakdownDetails.type")}:{" "}
                    <span className="text-gray-800">
                      {assetTypeDetails?.assignment_type ||
                  ...
                  {assetTypeDetails?.assignment_type === "User" && (
                    <p className="mt-1 text-xs text-gray-500">
                      {t("breakdownDetails.leaveEmptyForCurrentUser", {
                        userId:
                          user?.user_id ||
                          user?.emp_int_id ||
                          t("breakdownDetails.notAvailable"),
                      })}
                    </p>
                  )}
                  {assetTypeDetails?.assignment_type === "Department" && (
                    <p className="mt-1 text-xs text-gray-500">
                      {t("breakdownDetails.leaveEmptyForCurrentDepartment", {
                        deptId:
                          user?.dept_id || t("breakdownDetails.notAvailable"),
                      })}
                    </p>
                  )}
                </div>
          ...
          <div className="flex items-center justify-end gap-4 pt-6">
            ...
            {!isReadOnly && (
              <>
                ...
                >
                  {isSubmitting
                    ? t("breakdownDetails.submitting")
                    : t("breakdownDetails.reportBreakdown")}
                </button>
              </>
            )}
          </div>
