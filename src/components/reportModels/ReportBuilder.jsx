...
export default function ReportBuilder() {
  ...
  const applyView = (v) => {
    setSelectedReportId(v.reportId);
    setQuick(v.quick);
    ...
    <div className="min-h-screen bg-slate-50 p-5">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-12 gap-5">
          {/* Left: Catalog & Saved Views */}
          <aside className="col-span-3">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-3 mb-4 h-[470px] overflow-y-auto">
                ...
                {REPORTS.map((r) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-xl text-sm ${
                      selectedReportId === r.id ? "bg-[#143d65] text-white" : "hover:bg-slate-100"
                    }`}
                  >
            ...
            {filteredViews.length > 0 && (
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-3 h-[250px] overflow-y-auto">
                <SectionTitle>Saved Views</SectionTitle>
                <div className="space-y-2">
                  {filteredViews.map((v) => (
                    <div key={v.id} className="flex items-center justify-between border border-slate-200 rounded-xl px-3 py-2">
                      <div>
                        <div className="text-sm font-medium">{v.name}</div>
                        <div className="text-xs text-slate-500">{REPORTS.find((r) => r.id === v.reportId)?.name}</div>
                      </div>
                      <button onClick={() => applyView(v)} className="text-sm px-3 py-1 rounded-lg bg-[#143d65] text-white">
                        Load
                      </button>
                    </div>
                  ))}
